    ...
    <div className="space-y-8 sm:space-y-10 pb-16 animate-in fade-in duration-700 max-w-6xl mx-auto">
      ...
      <section className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between sm:gap-6 pt-1 sm:pt-4">
        <div className="space-y-0.5 sm:space-y-2">
          <h1 className="text-xl sm:text-3xl font-bold tracking-tight text-[#1C2A3B]">
            Dashboard
          </h1>
          <p className="text-xs sm:text-sm text-[#8E959D] font-medium flex items-center gap-2">
            Sessions and insights at a glance.
            {teamInfo && (
              <span className="text-[9px] font-semibold bg-[#E4E1FE] text-[#786BF9] px-2 py-0.5 rounded-full">
      ...
      <section className="grid grid-cols-4 gap-2 sm:grid-cols-2 sm:gap-4 lg:grid-cols-4">
        {stats.map((stat, i) => (
          <div key={i} className="p-3 sm:p-6 bg-white border border-[#E8EAEB] rounded-2xl space-y-1 sm:space-y-3 hover:shadow-sm transition-all group">
             <stat.icon className="w-4 h-4 sm:w-5 sm:h-5 text-[#786BF9] group-hover:text-[#6056C7] transition-colors" />
             <div>
                <p className="text-lg sm:text-2xl font-bold text-[#1C2A3B]">{stat.value}</p>
                <p className="text-[8px] sm:text-[11px] text-[#8E959D] font-medium uppercase tracking-wider">{stat.title}</p>
             </div>
          </div>
        ))}
      </section>
