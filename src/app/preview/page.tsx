...
export default function PreviewPage() {
  ...
  const handleExportCSV = () => {
    ...
    exportToCSV(data, `MOSI_${session.stakeholder.company}.csv`)
  }
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap items-center gap-4 text-xs text-slate-400">
              <span className="flex items-center gap-1"><Building2 className="w-3.5 h-3.5" />{stakeholder.company}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          {activeTab === 'transcript' && (
            <div className="bg-white border border-slate-100 rounded-2xl p-6 space-y-4 shadow-sm animate-in fade-in duration-300">
              <div className="flex items-center justify-between">
              ...
              <div className="space-y-3">
                {transcript?.map((para) => (
                  <div key={para.id} className={cn("flex gap-4 p-4 rounded-xl transition-all group",
                    para.status === 'Approved' ? "bg-emerald-50/30" : 
                    para.status === 'Hidden' ? "opacity-20" : "hover:bg-slate-50"
                  )}>
                    <div className="w-20 shrink-0 text-right">
                      <p className="text-xs font-semibold text-slate-900">{para.speaker}</p>
                      <p className="text-[10px] text-slate-400">{formatDuration(para.timestamp)}</p>
                    </div>
                    <div className="flex-1">
                      <p className="text-sm text-slate-600 leading-relaxed">{para.text}</p>
                      {para.comment && (
                        <div className="mt-2 p-2 rounded-lg bg-amber-50 flex items-start gap-2">
                          <MessageCircle className="w-3 h-3 text-amber-500 shrink-0 mt-0.5" />
                          <p className="text-[10px] text-amber-600 italic">"{para.comment}"</p>
                        </div>
                      )}
                    </div>
                    <div className="flex items-start gap-1 opacity-0 group-hover:opacity-100 transition-all">
                      <button onClick={() => handleStatusUpdate(para.id, 'para', 'Approved')} className="p-1.5 rounded-lg border border-slate-200 text-slate-400 hover:text-emerald-500 bg-white"><Check className="w-3 h-3" /></button>
                      <button onClick={() => handleCommentUpdate(para.id, 'para')} className="p-1.5 rounded-lg border border-slate-200 text-slate-400 hover:text-blue-500 bg-white"><MessageSquare className="w-3 h-3" /></button>
                      <button onClick={() => handleStatusUpdate(para.id, 'para', para.status === 'Hidden' ? 'Pending' : 'Hidden')} className="p-1.5 rounded-lg border border-slate-200 text-slate-400 hover:text-red-500 bg-white">{para.status === 'Hidden' ? <Eye className="w-3 h-3" /> : <EyeOff className="w-3 h-3" />}</button>
                    </div>
                  </div>
                ))}
              </div>
